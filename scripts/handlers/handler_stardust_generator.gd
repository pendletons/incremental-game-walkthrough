...
func _ready() -> void:
	...
	HandlerCCUpgrades.ref.upgrade_01_stardust_generation.leveled_up.connect(
		watch_for_ccu01_level_up
	)
...
func watch_for_ccu01_level_up() -> void:
	timer.start()
	HandlerCCUpgrades.ref.upgrade_01_stardust_generation.leveled_up.disconnect(
		watch_for_ccu01_level_up
	)
